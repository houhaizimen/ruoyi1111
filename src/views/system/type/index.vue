<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="id对应的索引" prop="idx">
        <el-input
          v-model="queryParams.idx"
          placeholder="请输入id对应的索引"
          clearable
    ...
    <el-table v-loading="loading" :data="typeList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="id对应的索引" align="center" prop="idx" />
      <el-table-column label="类型名" align="center" prop="name" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="id对应的索引" prop="idx">
          <el-input v-model="form.idx" placeholder="请输入id对应的索引" />
        </el-form-item>
        <el-form-item label="类型名" prop="name">
          <el-input v-model="form.name" placeholder="请输入类型名" />
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除实体类型编号为"' + ids + '"的数据项？').then(function() {
        return delType(ids);
      }).then(() => {
        this.getList();
